<?php
class Centralexreport extends CI_Model{
...
foreach($datedata as $date){
  ...
  if($query9->num_rows() >0){
    foreach($query9->result_array() as $salesinfo){
      ...
      $salesqty=$salesqty+$salesinfo['saleqtysum'];
      $pack=$pack+$salesinfo['packsum'];
	 ...
	 function showData($sdate,$edate){
		...
		$result=array();
           ...
           $this->db->join('con-party_master', 'con-party_master.id = purchase_master.party_id');
				 $this->db->where('purchase_master.type','Finish Product');

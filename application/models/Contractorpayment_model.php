<?php
class Contractorpayment_model extends CI_Model{
    ...
    function showData($table){
		...
		$this->db->where('party','contractor');
		$queryin=$this->db->get()->result();
		foreach($queryin as $condata){
			$conname=$condata->name;
			$id=$condata->id;
			$bankac=$condata->bankac;
			$ifsc=$condata->ifsc;
       ...
       // $quantity=$this->db->get()->row()->qty;
		...
		//echo $grtotal;
		//	$fitotal=$grtotal-($tLev+$tTob+$whsutta+$blsutta+$bags+$dice+$tsort+$advance+$pf);
		$resultdata[]=array(
			'conname'=>$conname,
			'id'=>$id,
			'bankac'=>$bankac,
